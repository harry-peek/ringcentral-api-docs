...
To receive thread messaging notification events, [subscribe for one or more event filters](../../../notifications/event-filters/thread-messaging-events.md).
...
Due to the complexity and optimization reason, the system fires events with just the `lastModifiedTime` value in the event payload. Developers can use the event notification as a trigger to call the [sync message thread](threads-handling.md#sync-message-threads) or the [sync thread entries](messages-handling.md#sync-thread-entries-messages) API to get the actual data.
